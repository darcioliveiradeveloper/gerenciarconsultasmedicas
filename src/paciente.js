...
function listar(callback) {
  ...
  console.log("\n=== Lista de Pacientes ===");
  if (pacientes.length === 0) console.log("Nenhum paciente cadastrado.");
  else pacientes.forEach(p => console.log(`ID: ${p.id} | Nome: ${p.nome} | Data de Nascimento: ${p.dataNascimento}`));
  ...
  rl.question("Nome do paciente: ", (nome) => {
    ...
    function perguntarData() {
      rl.question("Data de nascimento (dd/mm/aaaa): ", (dataNascimento) => {
        ...
        if (!resultadoData.valido) { validacao.mostrarErro(resultadoData.mensagem); return perguntarData(); }
        ...
        validacao.confirmarAcao(rl, "Deseja confirmar o cadastro do paciente?", (ok) => {
          if (ok) { pacientes.push(novo); salvar(pacientes); console.log("✅ Paciente cadastrado!"); }
          callback();
        });
      });
    }
...
function atualizar(rl, callback) {
  ...
  rl.question("ID do paciente: ", (id) => {
    ...
    if (!paciente) { validacao.mostrarErro("Paciente não encontrado."); return callback(); }
    ...
    rl.question(`Novo nome (${paciente.nome}): `, (nome) => {
      ...
      function perguntarData() {
        rl.question(`Nova data de nascimento (${paciente.dataNascimento}): `, (dataNascimento) => {
          ...
          console.log("\n📋 Comparação:"); validacao.mostrarDados("Antigo", paciente); validacao.mostrarDados("Novo", novosDados);
          validacao.confirmarAcao(rl, "Deseja confirmar a atualização?", (ok) => {
            if (ok) { paciente.nome = novoNome; paciente.dataNascimento = novaData; salvar(pacientes); console.log("✅ Paciente atualizado!"); }
            callback();
          });
        });
      }
...
function remover(rl, callback) {
  ...
  rl.question("ID do paciente: ", (id) => {
    ...
    validacao.confirmarAcao(rl, "Deseja confirmar a exclusão?", (ok) => {
      if (ok) { pacientes = pacientes.filter(p => p.id != id); salvar(pacientes); console.log("🗑️ Paciente removido!"); }
      callback();
    });
  });
}
...
function buscarPorId(rl, callback) {
  ...
  rl.question("Digite o ID do paciente: ", (id) => {
    const paciente = pacientes.find(p => p.id == id);
    paciente ? validacao.mostrarDados("Paciente encontrado", paciente) : validacao.mostrarErro("Paciente não encontrado.");
    callback();
  });
}
function buscarPorNome(rl, callback) {
  ...
  rl.question("Digite o nome do paciente: ", (nome) => {
    const encontrados = pacientes.filter(p => p.nome.toLowerCase().includes(nome.toLowerCase()));
    encontrados.length ? encontrados.forEach(p => validacao.mostrarDados("Paciente encontrado", p)) : validacao.mostrarErro("Nenhum paciente encontrado.");
    callback();
  });
}
function buscarPorNascimento(rl, callback) {
  const pacientes = carregar();
  rl.question("Digite a data de nascimento (dd/mm/aaaa): ", (dataNasc) => {
    const encontrados = pacientes.filter(p => p.dataNascimento == dataNasc);
    encontrados.length ? encontrados.forEach(p => validacao.mostrarDados("Paciente encontrado", p)) : validacao.mostrarErro("Nenhum paciente encontrado.");
    callback();
  });
}

module.exports = { adicionar, listar, atualizar, remover, buscarPorId, buscarPorNome, buscarPorNascimento };
